...
function buildMatcher(mode: 'contains' | 'startsWith' | 'endsWith' | 'regex', pattern: string): StringMatcher {
 ...
 */
export async function enforceStorageBudget(maxBytes: number, maxDeletesPerRun: number = 200): Promise<{ deleted: number; freedBytes: number; totalAfter: number }>{
  let total = await getTotalStorageBytes();
  if (total <= maxBytes) return { deleted: 0, freedBytes: 0, totalAfter: total };

  let freed = 0;
  let deleted = 0;
  
  // Load protected filters (by tag or category)
  const protectFilters = await prisma.filter.findMany({ where: { active: true, type: 'protect' } });
  ...
  const isProtected = (doc: { tag?: string | null; category?: string | null }) => {
    return tagMatchers.some((m) => m(doc.tag)) || categoryMatchers.some((m) => m(doc.category));
  };
  ...
  while (total - freed > maxBytes) {
    ...
    const batch: { id: number; bytes: number | null; tag: string | null; category: string | null }[] = await prisma.document.findMany({
  ...
  cursor: cursorId ? { id: cursorId } : undefined,
    ...
    for (const d of batch) {
      ...
      // Detach CrawlUrls
      await prisma.crawlUrl.updateMany({ where: { documentId: d.id }, data: { documentId: null } });
      // Delete document
